...
echo "Starting server in recovery mode for $NODE_NAME..."
nohup $SERVER_BINARY --mode=recover --port=$PORT \
  --data-dir=$DATA_DIR --peers-file=$PEERS_FILE > "$LOG_FILE" 2>&1 &

SERVER_PID=$!
wait $SERVER_PID

# Check if recovery succeeded
if grep -q "$RECOVERY_SUCCESS_MARKER" "$LOG_FILE"; then
